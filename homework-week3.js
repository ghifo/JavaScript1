...
console.log(
  "This statement should add the new element 'meerkat' in the second position which has the index 1 without removing any other elements from the existing array.",
);
...
function vehicle(color, code, age) {
  if (age === 1 && code === 1) {
    return `a new ${color} car`;
  } else if (age === 1 && code === 2) {
    return `a new ${color} motorbike`;
  } else if (age > 1 && code === 1) {
    return `an old ${color} car`;
  } else if (age > 1 && code === 2) {
    return `an old ${color} motorbike`;
  }
}
...
listOfTeachers.languages = ['HTML/CSS', 'CLI', 'JavaScript1'];
console.log(listOfTeachers);
...
console.log(
  'the order that you assign (o3 = o2 or o2 = o3) matters because (o2 = o3) will gives syntax error.',
);
...
console.log(
  "It will return 'string' because it returns the typeof bar as a value for the second typeof expression",
);
